...
const tickets: any = computed(() => ticketsData.value)

const fetchTickets = async () => {
  try {
    isLoading.value = true
    const response: any = await useFetchApi(`/api/auth/tickets?page=${page.value}&pagesize=${pageSize.value}`);
    ticketsData.value = response?.data?.tickets;
...
const handleSearchData = async (query: string) => {
  try {
    ...
    isLoading.value = true
    const response: any = await useFetchApi(`/api/auth/tickets/search?q=${query}`);
    ticketsData.value = response?.data?.tickets;
    totalPages.value = 1;
    nextPage.value = null;
    prevPage.value = null;
...
const handleDeleteData = async (id: number) => {
  ...
  try {
    ...
    ticketsData.value = ticketsData.value.filter((user: any) => user.id !== id);
